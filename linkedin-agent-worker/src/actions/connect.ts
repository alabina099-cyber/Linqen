  ...
  try {
    ...
    if (note) {
      ...
      for (const sel of addNoteSelectors) {
        const noteClicked = await safeClick(page, sel);
        if (noteClicked) {
          await sleep(500);
    ...
    if (!sendClicked) {
      ...
      sendClicked = await page.evaluate(() => {
        const buttons = Array.from(document.querySelectorAll('button'));
        const btn = buttons.find(b =>
          b.innerText.trim().startsWith('Envoyer') ||
    ...
    const rateLimited =
      successText.includes('Vous avez atteint la limite') ||
