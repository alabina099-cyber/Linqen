...
function updateStats(pendingData, completedData) {
  ...
  document.getElementById('totalCount').textContent = total || (pending + completed);
}
...
function updateActionsList(actions) {
  ...
  list.innerHTML = actions.map(action => {
    ...
    const name = action.target_name || payload?.keywords || action.target_url?.split('/in/')[1] || 'Action';
