...
// Récupérer et exécuter les actions en attente
async function pollAndExecuteActions() {
  if (isProcessing) return;

  try {
    const response = await fetch(`${API_BASE_URL}/linkedin-actions?status=pending&limit=5`);
...
async function executeAction(action) {
  ...
  switch (action.action_type) {
    ...
    case 'send_message':
      ...
      break;
    default:
      await updateActionStatus(action.id, 'failed', null, `Type d'action inconnu: ${action.action_type}`);
  }
}
...
async function executeSearch(action) {
  try {
    ...
    if (!tab) {
      await updateActionStatus(action.id, 'failed', null, 'Aucun onglet LinkedIn ouvert. Ouvrez LinkedIn d\'abord.');
      return;
    }
...
async function executeVisitProfile(action) {
  try {
    const tab = await getLinkedInTab();
    if (!tab) {
      await updateActionStatus(action.id, 'failed', null, 'Aucun onglet LinkedIn ouvert.');
      return;
    }
...
async function executeSendMessage(action) {
  try {
    ...
    const messagingUrl = action.target_url.replace('/in/', '/messaging/thread/new/') || action.target_url;
    await chrome.tabs.update(tab.id, { url: action.target_url });
    await delay(3000);
...
async function updateActionStatus(actionId, status, result = null, errorMessage = null) {
  try {
