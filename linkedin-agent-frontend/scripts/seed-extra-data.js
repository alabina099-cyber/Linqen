...
const industries = [
  "Technology", "SaaS", "AI", "Data", "Finance", "Retail",
  "Healthcare", "Education", "Consulting", "Media", "Real Estate",
  "Energy", "Transport", "Food", "Legal", "Marketing",
  "Telecom", "Insurance", "Construction", "Automotive",
];
...
function generateProspectName(index) {
  const firstNames = [
    ...
    "Capucine", "Enzo", "Lilou", "Mathis", "Elena", "Nathan",
    "Julia", "Liam", "Lisa", "Gabriel", "Anna", "Adam",
    "Sarah", "Rayan", "Manon", "Youssef", "Fatima", "Omar",
    "Aïcha", "Karim", "Yasmin", "Mehdi", "Salma", "Amine",
    "Nadia", "Hamza", "Rania", "Tariq", "Lina", "Idriss",
    "Dounia", "Sofiane", "Samira", "Bilal", "Khadija", "Reda",
  ];
  const lastNames = [
    ...
    "Garnier", "Rousseau", "Blanc", "Guerin", "Muller", "Fontaine",
    ...
    "Aubert", "Denis", "Bourgeois", "Renard", "Lacroix", "Leclerc",
    "Girard", "Joly", "Picard", "Gaillard", "Roger", "Sanchez",
    "Dupuis", "Colin", "Leroux", "Clement", "Gauthier", "Sutherland",
    "Nguyen", "Lefort", "Dufour", "Rolland", "Moulin", "Jacquet",
    "Valentin", "Bouvier", "Lamy", "Rey", "Benoit", "Pons",
    "Hamel", "Delaunay", "Maillard", "Poulain", "Tessier", "Legros",
    "Weber", "Fernandez", "Lopes", "Imbert", "Chartier", "Pelletier",
    "Bouchet", "Maury", "Cohen", "Lejeune", "Klein", "Jacques",
    "Boulanger", "Prevost", "Marchal", "Carpentier", "Didier", "Becker",
    "Gosselin", "Michaud", "Lemoine", "Hervé", "Cordier", "Monnier",
    "Pereira", "Marques", "Ribeiro", "Ferreira", "Alves", "Santos",
    "Oliveira", "Silva", "Souza", "Lima", "Costa", "Martins",
    "Rodrigues", "Almeida", "Nascimento", "Carvalho", "Araujo", "Moreira",
    "Benoit", "Pelletier", "Gagnon", "Roy", "Tremblay", "Côté",
    "Gauthier", "Morin", "Lavoie", "Fortin", "Gagné", "Ouellet",
    "Petit", "Bouchard", "Poirier", "Lemieux", "Renaud", "Savard",
    "Lessard", "Hénault", "Bérubé", "Dubé", "Lachance", "Ménard",
    "Villeneuve", "Rivard", "Laplante", "Marchand", "Plante", "Bédard",
    "Bernier", "Maltais", "Dion", "Labonté", "Champagne", "Fortier",
    "Giroux", "Martel", "Blais", "Dufresne", "Couture", "Larose",
    "Laliberté", "Vachon", "Paquette", "Gervais", "Desjardins", "Beaudoin",
    "Lauzon", "Houde", "Lemire", "Perreault", "Gravel", "Thibault",
    "Gagné", "Beaulieu", "Paradis", "Boucher", "Lacroix", "Caron",
    "Lévesque", "Perron", "Lepage", "Rochon", "Goulet", "Audet",
    "Cyr", "Parent", "Giguère", "Langevin", "Pilon", "Jacques",
  ];
  return `${firstNames[index % firstNames.length]} ${lastNames[index % lastNames.length]}`;
}

async function seedExtraData() {
  console.log("🌱 Ajout de données supplémentaires pour ProspectMap et Heatmap...");
  ...
  try {
    ...
    const tools = ["search_prospects", "visit_profile", "send_connection", "send_message", "analyze_profile"];
    const toolValues = [];
    for (let i = 0; i < 50; i++) {
    ...
    const actionTypes = ["search", "visit_profile", "send_connection", "send_message"];
    ...
    for (let i = 0; i < 40; i++) {
      ...
      const executedAt = status === "completed" ? `NOW() - INTERVAL '${daysAgo} days'` : "NULL";
    ...
    // Vérification
    const [prospectsCount, messagesCount, toolStepsCount, actionsCount] = await Promise.all([
    ...
    for (let d = 0; d < 7; d++) {
      const dayData = heatmapStats.rows.filter((r) => parseInt(r.day_of_week) === d);
      if (dayData.length > 0) {
